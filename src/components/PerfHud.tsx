...
export default function PerfHud() {
  ...
  return (
    ...
    >
      <span className="text-[11px] font-semibold uppercase tracking-widest text-gray-500 dark:text-white/55">
        This page, as served
      </span>
      <span className="mt-2 flex gap-4">
        {chips.map(([value, label]) => (
          <span key={label} className="flex flex-col">
            <span className="font-mono text-sm font-semibold text-gray-900 tabular-nums dark:text-white">
              {value}
            </span>
            <span className="text-[11px] text-gray-500 dark:text-white/55">
              {label}
            </span>
          </span>
        ))}
      </span>
      <span className="mt-2 block text-xs text-emerald-700 transition-transform duration-200 group-hover:translate-x-0.5 dark:text-emerald-400">
